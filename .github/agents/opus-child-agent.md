...
| **リクエスト名**           | 出力ファイル名に使用する識別子                                   | `パス受け渡しルール明確化`                                 |
| **DOCS_ROOTの状態**        | 親が環境変数を確認した結果（展開済み絶対パス、または「未設定」） | `/docs` または `未設定`                                    |
| **リクエスト実行履歴パス** | リクエスト実行履歴.mdの絶対パス（リクエストフォルダ配下・親が管理） | `/docs/main/tasks/devcontainer/リクエスト名/リクエスト実行履歴.md` |

### 成果物出力情報

| 情報                   | 説明                                                   | 例                                                      |
| ---------------------- | ------------------------------------------------------ | ------------------------------------------------------- |
| **成果物出力先パス**   | プロセス別のディレクトリパス（絶対パス、親が作成済み） | `/docs/main/tasks/devcontainer/リクエスト名/001-2/調査/` |
...
```
{DOCS_ROOT}/{ブランチ名}/tasks/{ワークスペースフォルダ名}/{リクエスト名}/{実行番号}/{プロセス}/
```
...
- **プロセス種別が未提供**: 作業内容から適切なプロセスを推測（推測できない場合は `child-{リクエスト名}.md`）
- **作業ディレクトリが未提供**: カレントディレクトリを使用する
- **リクエスト名が未提供**: 作業内容から適切な名前を推測して使用する
- **DOCS_ROOTが「未設定」と通知された場合**: 提供された情報を信頼し、成果物出力をスキップする（自分で `echo $DOCS_ROOT` を実行して確認しない）
- **リクエスト実行履歴パスが未提供**: 特に対応は不要。親エージェントが履歴を管理している
...
- 出力先パス: `/docs/main/tasks/devcontainer/リクエスト名/001-2/`
- ファイル名: `child-リクエスト名.md`
- 完全なパス: `/docs/main/tasks/devcontainer/リクエスト名/001-2/child-リクエスト名.md`
...
- 成果物出力先パス: `/docs/main/tasks/devcontainer/リクエスト名/001-2/調査/`
- 成果物ファイル名: `調査レポート.md`
- 完全なパス: `/docs/main/tasks/devcontainer/リクエスト名/001-2/調査/調査レポート.md`
...
- **プロセス別デフォルト**:
  ...
  - 計画プロセス: `計画書.md`
  - 実行プロセス: `実装レポート.md`
- **汎用フォーマット**: `child-{リクエスト名}.md`
- リクエスト名は親エージェントから指定されたものを使用
- 日本語のファイル名もそのまま使用可能
...
| プロセス種別が不明       | 汎用形式 `child-{リクエスト名}.md` で出力                      |
...
- 期待される成果物: 調査レポート.md
- DOCS_ROOT: /docs
- リクエスト実行履歴: /docs/main/tasks/myproject/API実装/リクエスト実行履歴.md （親エージェントが管理）
