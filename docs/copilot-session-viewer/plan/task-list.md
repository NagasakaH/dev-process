...
| task02 | terminal.ts captureTmuxPane `-e` フラグ拡張 | task01 | G2-A | 10分 | UT-T1,T2,T3 + 既存テスト修正 |
...
| task04-01 | server.js + setupTerminalWebSocket 実装 | task03-03 | G5 | 30分 | — (task04-02でテスト) |
| task04-02 | 結合テスト IT-1〜IT-11 | task04-01 | G6 | 30分 | IT-1〜11 |
| task05-01 | useTerminalWebSocket フック | task03-01 | G3 | 15分 | UT-29〜33 |
| task05-02 | TerminalView + TerminalModal コンポーネント | task05-01 | G5 | 25分 | UT-21〜28 (Modal+View) |
| task06 | ActiveSessionsDashboard 統合（AC-5 実装担当） | task05-02 | G7 | 10分 | — |
| task07 | デプロイ設定 (Dockerfile + scripts) | task04-02 | G7 | 10分 | — |
...
graph TD
    ...
    task04_01["task04-01<br/>server.js+setupTerminalWebSocket"]
    task04_02["task04-02<br/>結合テスト IT-1〜11"]
    ...
    style task04_01 fill:#fce4ec
    style task04_02 fill:#fce4ec
    style task05_02 fill:#fce4ec
...
| G5 | task04-01, task05-02 | server 実装とコンポーネント → 並列可 |
| G6 | task04-02 | 結合テスト（task04-01 完了後） |
...
### 単体テスト (terminal.test.ts)

| テストID | タスク | テスト内容 |
|----------|--------|-----------|
| UT-T1 | task02 | withEscape=true → -e フラグ付き |
| UT-T2 | task02 | withEscape=false（デフォルト）→ -e なし |
| UT-T3 | task02 | Docker exec + withEscape=true |
...
### 単体テスト (useTerminalWebSocket.test.ts)

| テストID | タスク | テスト内容 |
|----------|--------|-----------|
| UT-29 | task05-01 | sessionId null → 未接続 |
| UT-30 | task05-01 | connecting → connected 状態遷移 |
| UT-31 | task05-01 | 再接続（指数バックオフ 1s→2s→4s、最大3回） |
| UT-32 | task05-01 | KeepAlive ping 送信 + pong タイムアウト |
| UT-33 | task05-01 | error メッセージ → error 状態遷移 |
...
| テストID | タスク | テスト内容 |
|----------|--------|-----------|
| IT-1 | task04-02 | WS接続→connected メッセージ |
| IT-2 | task04-02 | capture-pane→WS出力 |
| IT-3 | task04-02 | WS入力→send-keys |
| IT-4 | task04-02 | 認証→WS接続拒否 |
| IT-5 | task04-02 | WS切断→クリーンアップ |
| IT-6 | task04-02 | Docker環境capture+send |
| IT-7 | task04-02 | 接続再確立 |
| IT-8 | task04-02 | resize→tmux resize-pane |
| IT-9 | task04-02 | 認証未設定時接続拒否 |
| IT-10 | task04-02 | ローカル総接続上限 |
| IT-11 | task04-02 | Docker総接続上限 |

### E2E テスト (terminal-viewer-basic.spec.ts)
...
### E2E テスト (terminal-viewer-auth.spec.ts)
...
| AC-2: リアルタイム表示 | UT-7,8 / IT-2 / E2E-2 | task03-02, task04-01 | task03-02, task04-02, task08-01 |
| AC-3: キー入力送信 | UT-10〜15 / IT-3 / E2E-3 | task03-02, task04-01 | task03-02, task04-02, task08-01 |
| AC-4: Docker対応 | UT-8,15 / IT-6 / E2E-5 | task03-02, task04-01 | task03-02, task04-02, task08-01 |
| AC-5: 既存機能正常 | E2E-6, E2E-7 | task06 | task08-02 |
