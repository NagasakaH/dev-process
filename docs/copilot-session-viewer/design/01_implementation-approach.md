...
| Docker exec 方式の統一 | bash -c パターンと execFileSync 直接実行の混在はサニタイズ責務が曖昧になる | Docker 環境でのコマンド実行は全て `execFileSync('docker', ['exec', ...])` で直接実行に統一する。`bash -c` 経由のパターンは使用しない（MRD-004） |
| 同時接続数上限 | 環境ごとのリソース制約が異なる | ローカル環境: 最大 5 接続、Docker 環境: 最大 2 接続に制限する。イベントループブロッキング防止のため環境別に上限を設定（MRD-006） |
...
| 2025-07-17 | 1.1 | MRD-004: Docker exec方式統一、MRD-006: 同時接続数上限追加 | Copilot |
