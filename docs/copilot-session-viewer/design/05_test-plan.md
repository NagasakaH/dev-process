...
| tmux セッションが予期せず終了しない | E2E | E2E-3, E2E-6 |
| .env から PAT を含む認証設定を供給できる | Unit + E2E | UT-9, UT-10, UT-11, E2E-4, E2E-7 |
...
describe("findDockerContainers", () => {
  ...
  it("should return empty array when DISABLE_DOCKER_DETECTION=true", async () => {
    vi.stubEnv("DISABLE_DOCKER_DETECTION", "true");
    // vi.resetModules() + dynamic import() to re-evaluate module-level const
    const { findDockerContainers } = await import("@/lib/terminal");
    expect(findDockerContainers()).toEqual([]);
  });

  it("should call execSync when DISABLE_DOCKER_DETECTION is not set", async () => {
...
> **MRD-009 対応**: `DISABLE_DOCKER_DETECTION` はモジュールスコープの `const` で評価されるため、
> `process.env` の変更だけではテスト間で値が切り替わらない。`vi.resetModules()` で
> モジュールキャッシュをクリアし、`await import()` で動的に再インポートすることで、
> 各テストで異なる環境変数値を確実に反映させる。
...
| E2E-6 | tmux 耐久性（5分間） | 1. コンテナ起動 2. 5分間待機（定期的に tmux 操作実行） 3. tmux セッション再確認 4. tmux detach/再接続 | セッション維持、操作正常、再接続成功 |
| E2E-7 | GITHUB_TOKEN 供給確認 | 1. .env に GITHUB_TOKEN 設定 2. コンテナ起動 3. `docker exec ... printenv GITHUB_TOKEN` | 環境変数がコンテナ内で利用可能 |
...
| 2026-03-21 | 1.2 | MRD-004: acceptance_criteria マッピング修正（UT-9,10,11,E2E-4,E2E-7）。MRD-007: tmux 5分間耐久テスト追加。MRD-009: vi.resetModules() + 動的 import() パターン明記 | Copilot |
