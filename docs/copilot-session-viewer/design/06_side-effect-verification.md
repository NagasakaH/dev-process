...
| 項目 | 確認内容 | 検証方法 | チェック |
|------|----------|----------|----------|
| WS 認証（設定時） | Basic Auth 設定時に正しい認証情報で接続可能、不正時は 401 で拒否 | UT-1, UT-2 + E2E-8 | ⬜ |
| WS 認証（未設定時） | Basic Auth 環境変数未設定時は 403 で接続拒否（ターミナル機能無効化） | UT-24-r + IT-9 | ⬜ |
| WS 認証バイパス | 認証なしで /ws/terminal に接続不可 | E2E-9, E2E-10 | ⬜ |
...
| DoS（同一pane制限） | 同一 pane 接続数制限が機能 | UT-19 | ⬜ |
| DoS（総接続上限） | 環境別の総接続数上限が機能（ローカル5、Docker2） | UT-25-r, UT-26-r, IT-10, IT-11 | ⬜ |
...
| 2025-07-18 | 1.2 | MRD2-001: セキュリティ検証表を401/403使い分けに更新+総接続上限検証追加 | Copilot |
