...
18. `npx msw init public/` を実行してService Workerファイルを生成
19. 生成された `public/mockServiceWorker.js` をコミット対象に含める
...
| `public/mockServiceWorker.js` | 自動生成 | MSW Service Worker（`npx msw init public/` で生成、コミット対象） |
...
- **並列実行時の競合回避 (MRP-007)**: 本タスクはtask02-01, task02-02と並列実行される。編集対象は `package.json`, `index.html`, `vite.config.ts`, `tsconfig*.json`, `eslint.config.js`, `.prettierrc`, `.gitignore`, `src/`配下, `public/`配下のみであり、`scripts/`配下には触れないこと。worktreeを分離して作業する。
...
- `public/mockServiceWorker.js` は `npx msw init public/` で生成し、コミット対象に含める（MRP-004対応）
