...
> **(RP2-007) 唯一の正規手順**: 下記順序は `scripts/web-e2e.sh` および task07/task10 と完全一致させる。順序逸脱・手順省略は禁止。
...
# 4. API Gateway デプロイメントを反映し invoke_url を確定 (RP-008)
scripts/apply-api-deployment.sh

# 5. 初回コールドスタート flaky を排除するため Lambda を warmup (RP-008)
scripts/warmup-lambdas.sh

# 6. invoke_url を assets/config.json に注入 + Angular ビルド
scripts/build-frontend.sh

# 7. floci S3 へ成果物配置（配置経路の検証のみ。ブラウザ配信は nginx host volume mount の唯一標準経路で行う）
