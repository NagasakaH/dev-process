...
3. `.gitlab-ci.yml` に下記を追加 (`02_interface-api-design.md` §7 を厳守):
   ...
   - `web-e2e`: stage `e2e` / 同 image / **DinD service** + `DOCKER_HOST=tcp://docker:2375` / `DOCKER_TLS_CERTDIR=""` / `FF_NETWORK_PER_BUILD: "true"` / `WEB_BASE_URL: "http://docker:8080"` / `AWS_ENDPOINT_URL: "http://docker:4566"` / `AWS_*=test` /
     - **before_script (RP2-001 解消・固定手順)**: `apt-get update` → `apt-get install -y --no-install-recommends docker.io docker-compose-plugin awscli unzip curl gnupg ca-certificates` → **Terraform 1.6.6 を固定インストール** (`curl -fsSL -o /tmp/tf.zip https://releases.hashicorp.com/terraform/1.6.6/terraform_1.6.6_linux_amd64.zip && unzip -o /tmp/tf.zip -d /usr/local/bin && terraform -version | head -1 | grep -q '1.6.6'`) → **.NET SDK 8.0 を固定インストール** (Microsoft 公式手順: `curl -fsSL https://packages.microsoft.com/config/ubuntu/22.04/packages-microsoft-prod.deb -o /tmp/ms.deb && dpkg -i /tmp/ms.deb && apt-get update && apt-get install -y --no-install-recommends dotnet-sdk-8.0 && dotnet --list-sdks | grep -q '^8\.'`) → `docker info` → `bash scripts/check-test-env.sh e2e`（RP2-001: e2e プロファイルが要求する terraform/dotnet を CI 上で確実に提供する）
     - **script (RP2-002 / RP3-002 解消・二重実行禁止)**: `docker compose -f compose/docker-compose.yml up -d floci nginx` → `bash scripts/web-e2e.sh` の **2 行のみ**。`check-test-env.sh` は **before_script のみで実行** し、`script:` では呼ばない。`web-e2e.sh` 内部での重複チェックを避けるため `variables:` または `script:` 冒頭で **`export SKIP_ENV_CHECK=1`** を設定する。`deploy-local.sh` / `apply-api-deployment.sh` / `warmup-lambdas.sh` / `build-frontend.sh` / `deploy-frontend.sh` / `wait-floci-healthy.sh` を **CI script から直接呼ばない**（`web-e2e.sh` が唯一のエントリポイントで内部実行する。RP-008 の順序保証は `web-e2e.sh` 側に集約）
     - after_script `docker compose down -v || true` / artifacts `frontend/test-results/, frontend/playwright-report/` + `reports.junit`
...
- [ ] **(RP2-001)** `web-e2e` の before_script で Terraform 1.6.6 と .NET SDK 8.0 が固定インストールされ、`terraform -version | head -1 | grep -q '1.6.6'` と `dotnet --list-sdks | grep -q '^8\.'` が成功する。続けて `bash scripts/check-test-env.sh e2e` が exit 0
- [ ] **(RP2-002 / RP3-002)** `web-e2e` の `script:` ブロックに含まれる行は **`docker compose ... up -d floci nginx` / `bash scripts/web-e2e.sh` の 2 行のみ**。`check-test-env.sh` は before_script だけで呼ばれること（script に含めない）。`SKIP_ENV_CHECK=1` を CI で export して `web-e2e.sh` 内部の重複チェックを回避していること。`deploy-local.sh` / `apply-api-deployment.sh` / `warmup-lambdas.sh` / `build-frontend.sh` / `deploy-frontend.sh` / `wait-floci-healthy.sh` を CI script から直接呼ばない（`grep -E 'deploy-local|apply-api-deployment|warmup-lambdas|build-frontend|deploy-frontend|wait-floci-healthy' .gitlab-ci.yml | grep -v '^#'` が web-e2e ジョブ範囲で 0 行）
- [ ] **(RP2-002)** `web-e2e.sh` を唯一のエントリポイントとして使用する旨を `.gitlab-ci.yml` のコメントに明記
