...
3. `.gitlab-ci.yml` に下記を追加 (`02_interface-api-design.md` §7 を厳守):
   ...
   - `web-e2e`: stage `e2e` / 同 image / **DinD service** + `DOCKER_HOST=tcp://docker:2375` / `DOCKER_TLS_CERTDIR=""` / `FF_NETWORK_PER_BUILD: "true"` / `WEB_BASE_URL: "http://docker:8080"` / `AWS_ENDPOINT_URL: "http://docker:4566"` / `AWS_*=test` /
     ...
     - **script (RP2-002 解消・二重実行禁止)**: `bash scripts/check-test-env.sh e2e` → `docker compose -f compose/docker-compose.yml up -d floci nginx` → `bash scripts/web-e2e.sh` の **3 行のみ**。`deploy-local.sh` / `apply-api-deployment.sh` / `warmup-lambdas.sh` / `build-frontend.sh` / `deploy-frontend.sh` / `wait-floci-healthy.sh` を **CI script から直接呼ばない**（`web-e2e.sh` が唯一のエントリポイントで内部実行する。RP-008 の順序保証は `web-e2e.sh` 側に集約）
...
- [ ] **(RP2-002)** `web-e2e` の `script:` ブロックに含まれる行は **`bash scripts/check-test-env.sh e2e` / `docker compose ... up -d floci nginx` / `bash scripts/web-e2e.sh` の 3 行のみ**。`deploy-local.sh` / `apply-api-deployment.sh` / `warmup-lambdas.sh` / `build-frontend.sh` / `deploy-frontend.sh` / `wait-floci-healthy.sh` を CI script から直接呼ばない（`grep -E 'deploy-local|apply-api-deployment|warmup-lambdas|build-frontend|deploy-frontend|wait-floci-healthy' .gitlab-ci.yml | grep -v '^#'` が web-e2e ジョブ範囲で 0 行）
