...
6. **テスト環境準備状況**: 実行環境でテストに必要なリソースの確認（不足時は `ask_user` でユーザーに要求）
7. **レビューサマリー**: 総合判定と指摘事項一覧
...
└── {target}/
    └── review-design/
        ...
        ├── 06_test-environment-readiness.md # テスト環境準備状況
        └── 07_review-summary.md            # レビューサマリー
```
...
- docs/{target}/review-design/06_test-environment-readiness.md
- docs/{target}/review-design/07_review-summary.md
