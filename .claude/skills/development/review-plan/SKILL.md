---
name: review-plan
description: タスク計画の妥当性をレビューするスキル。setup.yaml + design-document + dev-plan/ + dev-design/を入力として、タスク分割の妥当性、依存関係の正確性、見積もりの妥当性、TDD方針の適切性を検証し、docs/{target_repo}/review-plan/ディレクトリにレビュー結果を出力、design-documentのレビューセクションを更新する。「review-plan」「計画レビュー」「計画をレビュー」「計画の妥当性」「plan review」「タスク計画チェック」「計画検証」などのフレーズで発動。dev-planスキル完了後、dev-implement実行前に使用。
---

# 計画レビュースキル（review-plan）

setup.yaml + design-document + dev-plan/ + dev-design/を入力として、タスク計画の妥当性を体系的にレビューし、レビュー結果をドキュメント化します。
...
3. **design-document** と **dev-plan/** からタスク計画を読み込み
4. **dev-design/** から設計内容を読み込み（計画が設計に基づいているか検証）
...
dev-designスキルで生成された詳細設計（計画が設計に基づいているかの検証に使用）：
...
| レベル | 説明 | 対応 |
|--------|------|------|
| 🔴 Critical | 計画を根本的に見直す必要がある | 差し戻し：dev-planの再実施が必要 |
...
| 判定 | 条件 | 次のステップ |
|------|------|-------------|
| ✅ 承認 | Critical/Majorの指摘なし | dev-implementスキルへ進行 |
| ⚠️ 条件付き承認 | Majorの指摘あり、Criticalなし | 指摘事項を修正後、再レビュー |
| ❌ 差し戻し | Criticalの指摘あり | dev-planスキルの再実施 |
...
### 次のステップ
1. ✅ 承認の場合: dev-implementスキルで実装を開始
2. ⚠️ 条件付き承認の場合: 指摘事項を修正後、再レビュー
3. ❌ 差し戻しの場合: dev-planスキルでタスク計画を再作成
```
...
```
エラー: タスク計画が見つかりません
ディレクトリ: docs/{target_repo}/dev-plan/

dev-planスキルでタスク計画を作成してください。
```

## 注意事項

- レビューは `target_repositories` の計画のみ対象
- dev-plan/が存在しない場合はエラー終了
- dev-design/が存在しない場合は警告を出し、設計との整合性チェックをスキップ
...
- 前提スキル: `dev-design` - 設計
- 前提スキル: `dev-plan` - タスク計画
- 後続スキル: `dev-implement` - 実装
...
[design-document確認] --> docs/{ticket_id}.md の存在確認
        |
[dev-plan/読み込み] --> タスク計画の読み込み
        |
[dev-design/読み込み] --> 設計結果の読み込み（任意）
        |
