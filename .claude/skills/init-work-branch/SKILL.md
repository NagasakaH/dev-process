...
- **task_name** - タスク名
- **target_repositories** - 修正対象リポジトリ（少なくとも1つ）。各リポジトリに `name`, `url`, `base_branch`（デフォルト: main）を含む。`submodules/editable/` 配下に配置

### オプション入力
- **description** - タスクの説明（overview, purpose, background, requirements, acceptance_criteria, scope, out_of_scope, notes）
- **related_repositories** - 参照用リポジトリ一覧（読み取り専用、`submodules/readonly/` 配下に配置）
...
- サブモジュール（`submodules/editable/` および `submodules/readonly/` 配下）
...
3. **サブモジュールディレクトリ準備** - `submodules/editable/` と `submodules/readonly/` を確認
4. **関連リポジトリ追加** - related_repositories を `submodules/readonly/` にサブモジュールとして追加
5. **修正対象リポジトリ追加** - target_repositories を `submodules/editable/` にサブモジュールとして追加、内部でfeatureブランチ作成
