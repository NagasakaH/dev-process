---
name: implement
description: planで作成されたタスク計画に従って実装を実行するスキル。setup.yaml + design-document + plan/を入力として、タスク実装管理、worktree管理、コミット管理、cherry-pick統合を行う。「implement」「実装を実行」「タスク実装」「計画を実行」「実装開始」などのフレーズで発動。planスキル完了後に使用。
---
...
```mermaid
flowchart TD
    A[plan/配下のタスク一覧読み込み] --> B[依存関係から実行順序・並列グループを特定]
    B --> C{タスク種別判定}
...
TASK_LIST_PATH="docs/${repo}/plan/task-list.md"
...
IMPL_LOG_PATH="${REPO_ROOT}/docs/${repo}/implement/execution-log.md"
