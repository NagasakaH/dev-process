...
| 🟡 Minor    | コードスタイル・軽微な改善点 | 承認：後で対応可能 |
| 🔵 Info     | 情報・提案・ベストプラクティスの紹介 | 承認：参考情報として記録 |

### 総合判定

| 判定           | 条件                          | 次のステップ                 |
| -------------- | ----------------------------- | ---------------------------- |
| ✅ 承認         | Critical/Majorの指摘なし      | finishing-branchへ進行       |
...
code_review:
  ...
  latest_verdict: "承認"
  summary: "Critical/Major指摘なし。Minor 2件は後続対応。"
  review_checklist:
    design_conformance:
      - id: DC-01
        name: "設計成果物との整合性"
...
- `design.artifacts`（設計準拠性チェック用）
- `implement.base_sha` / `implement.head_sha`（コミット範囲）
- `code_review.round`（存在する場合、再レビュー）
- `code_review.issues`（再レビュー時、前回指摘の確認）
...
`code_review` セクションを更新（round, status, review_checklist, issues, rounds, artifacts）。
`meta.updated_at` も更新。
