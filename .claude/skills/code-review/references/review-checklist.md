...
| TC-08 | AC全項目テスト必須 | 🔴 **Critical**: 全ACに対応するテストが存在し、かつpassしていること。未達は自動rejected |
| TC-09 | 修正範囲スコープ | 🔴 **Critical**: 修正がDR合意済みリポジトリ（editable/配下）に限定されているか |

### TC-08: Acceptance Criteria テスト完全性（Critical）

**これは絶対遵守の項目です。いかなる事情があっても、全ACのテストが完了していなければ approved にしてはなりません。**

確認手順:
1. DR MR/PRのACテストマッピングテーブルを取得
2. 各ACに対応するテストの実行結果を確認
3. 全ACに対してテストがpassしていることを検証
4. テストに必要なリソース（環境・データ等）が不足する場合は **ユーザーに提供を要求**

判定:
- 全ACテストpass → ✅
- 1つでもテスト未実施/fail → 🔴 Critical（自動rejected）
- テスト実行に必要なリソース不足 → ユーザーに `ask_user` で要求

### TC-09: 修正範囲スコープ検証（Critical）

確認手順:
1. DR MR/PRの「修正対象リポジトリ合意テーブル」を取得
2. 実際に変更されたファイルのリポジトリを確認
3. `submodules/readonly/` 配下のリポへの変更がないことを確認

判定:
- 合意済みリポのみ → ✅
- 合意外リポへの変更あり → 🔴 Critical（自動rejected）
