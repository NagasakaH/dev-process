...
3. **ユーザー対話が必須のスキルはこのエージェントが直接実行する**:
   以下のスキルは、利用可能な対話用ツール（`ask_user` または `askQuestions`）を使ったユーザーとの段階的対話が必要なため、サブエージェントではなくこのエージェント自身が直接実行すること：
   - **create-setup-yaml** — ユーザーと対話しながら setup.yaml を0から作成（段階的質問が必須）
   - **brainstorming** — ユーザーに質問を投げかけて要件を明確化し、利用可能な対話用ツールでテスト戦略を確認
...
12. **ユーザー確認は利用可能な対話用ツールで行う**: 対話が必要な場面では必ず利用可能な対話用ツール（`ask_user` または `askQuestions`）を使用してユーザーに確認を取る。テキスト出力だけで確認を取ったことにしてはならない
...
- 両方で失敗 → ユーザーに利用可能な対話用ツール（`ask_user` または `askQuestions`）で報告し判断を仰ぐ
...
- 利用可能な対話用ツール（`ask_user` または `askQuestions`）によるユーザー対話
...
**ワークフロー開始直後に、ユーザーにテスト範囲を利用可能な対話用ツール（`ask_user` または `askQuestions`）で確認する。**

この確認はスキップ不可。以下の情報を明確にする：

```
利用可能な対話用ツールを使用して以下を確認:
...
### 対話は利用可能な対話用ツールで行う（必須）

**全ての対話は利用可能な対話用ツール（`ask_user` または `askQuestions`）を使用して行うこと。** テキスト出力で質問し、次のメッセージで回答を待つ形式は禁止。利用可能な対話用ツールは選択肢を提示してユーザーの回答を確実に取得できる。
...
| ステップ                   | 対話内容                               | ツール   |
| -------------------------- | -------------------------------------- | -------- |
| Step 0 (create-setup-yaml) | タスク情報、要件、リポジトリの聞き取り | ask_user / askQuestions |
| Step 0.5 (テストスコープ)  | テスト範囲の確認（単体/結合/E2E）      | ask_user / askQuestions |
| Step 3 (brainstorming)     | 要件の深掘り、設計方針の決定           | ask_user / askQuestions |
| Step 7 完了後              | 実装結果の確認、追加タスクの有無       | ask_user / askQuestions |
| Step 8 完了後              | 検証結果の確認、追加検証の必要性       | ask_user / askQuestions |
| Step 10 (finishing-branch) | マージ/PR/保持/破棄の選択              | ask_user / askQuestions |
| 各ステップ完了時           | 次のステップに進んでよいかの確認       | ask_user / askQuestions |

### 中断前の確認

**作業を中断する前に、必ず利用可能な対話用ツール（`ask_user` または `askQuestions`）を使って以下を提示してユーザーの確認を取ってください：**

```
対話用ツールの使用例（`ask_user` 形式。`askQuestions` しか存在しない環境では同等の引数に読み替える）:
...
allow_freeform: true  # ask_user ではこの設定を使う。askQuestions では同等設定を使う
```
...
4. 両モデルで失敗した場合はユーザーに利用可能な対話用ツール（`ask_user` または `askQuestions`）で報告し、選択肢を提示：
   - 手動修正してから再開
   - このステップをスキップ（非推奨）
   - 作業を中断
