...
- 各ワークフローステップの実行には対応するスキルの `skill` ツール呼び出しが**必須**
- スキルを使わない手動実行は品質保証機能（テンプレート適用、チェックリスト検証等）がバイパスされるため**禁止**
- サブエージェントからも `skill` ツール呼び出しが可能であることが検証済み

各ワークフローステップの手順:
1. `prompts/workflow/{step}.md` を参照しコンテキスト取得手順を確認
2. `project-state` スキルで project.yaml から情報を抽出
3. 対応する汎用スキルを `skill` ツール経由で実行（手動実行は禁止）
